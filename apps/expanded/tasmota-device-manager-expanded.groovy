...
String getDeviceInfoByName(infoName) { 
    ...
    Map deviceInfo = ['name': 'Tasmota Device Manager', 'namespace': 'tasmota', 'author': 'Markus Liljergren', 'filename': 'tasmota-device-manager']
...
void deviceDescriptionHandler(hubitat.device.HubResponse hubResponse) {
	...
	if (body?.device?.manufacturer?.text().startsWith("iTead") || body?.device?.manufacturer?.text().startsWith("Royal")) {
		def devices = getDevices()
		def device = devices.find {it?.key?.contains(body?.device?.UDN?.text())}
    if (!device) {
            String hwaddr = (body?.device?.UDN?.text())[-12..-1].toUpperCase()
            device = devices.find {it?.value.mac?.contains(hwaddr)}
        }
		if (device) {
